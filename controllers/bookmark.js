...
export default {
  list: async (req, res, next) => {
    ...
    try {
      ...
      return res.status(200).send({ message: "Bookmarks retrieved", total, data: bookmarks });
    } catch (err) {
      next(err);
    }
  },
  ...
  create: async (req, res, next) => {
    ...
    try {
      ...
      if(isBookmarked) {
        return res.status(400).send({ message: "Post has been bookmarked before" })
      }
      ...
      return res.status(201).send({ message: "Bookmark created", data: bookmark });
    } catch (err) {
      next(err);
    }
  },
