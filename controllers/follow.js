...
export default {
  list: async (req, res, next) => {
    ...
    const filter = {};
    if (type === "followers")
      filter.targetId = userId === "me" ? req.user._id : userId;
    if (type === "followings")
      filter.doerId = userId === "me" ? req.user._id : userId;
    ...
    try {
      ...
      return res
        .status(200)
        .send({ message: "Follows retrieved", total, data: follows });
    } catch (err) {
      next(err);
    }
  },
  ...
  create: async (req, res, next) => {
    ...
    if (doerId === targetId) {
      return res
        .status(400)
        .send({ message: "Self-following is not permitted" });
    }
