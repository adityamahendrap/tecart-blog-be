...
const authService = {
  ...
  login: async (email, pasword) => {
    try {
      ...
      const validPassword = await authService.compareHash(user.password, pasword)
      if (!validPassword) {
  ...
  resetPassword: async (email) => {
    try {
      ...
      if(!emailSended) {
        ...
        throw new ResponseError(500, "Failed sending email reset password, please try again")
      }

      const hashedResetPassword = await authService.hashPassword(resetPassword)
      const result = await User.updateOne({ email }, { password: hashedResetPassword })
  ...
  changePassword: async (userId, currentPassword, newPassword) => {    
    try {
      ...
      const validCurrentPassword = await authService.compareHash(user.password, currentPassword)
      if(!validCurrentPassword) {
        throw new ResponseError(400, 'Invalid current password')
      }
      if(currentPassword === newPassword) {
        throw new ResponseError(400, 'The current password and the new password are still the same')
      }

      const hashedNewPassword = await authService.hashPassword(newPassword);
      const result = await User.updateOne({ userId }, { password: hashedNewPassword })
  ...
  githubOathToken: async (code)  => {
    ...
    try {
      ...
      logger.error('sessionService.getGithubOathToken -> Success get github oauth token')
      return decoded;
    } catch (err) {
